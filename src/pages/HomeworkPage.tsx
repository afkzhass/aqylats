...
const statusLabels: Record<string, { label: string; color: string; icon: any }> = {
  ...
  ai_reviewed: { label: "AI проверено", color: "bg-purple-100 text-purple-700", icon: AlertCircle },
...
const HomeworkPage = () => {
  ...
  const fetchSubmissions = async (assignmentId: string) => {
    ...
    if (data && data.length > 0) {
      ...
      setSubmissions(data.map(s => ({ ...s, profile: profiles?.find(p => p.user_id === s.student_id) })));
    } else {
      setSubmissions(data || []);
    }
  };
          ...
          <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
            ...
            <DialogContent className="max-w-md">
              ...
              <div className="space-y-3">
                ...
                <Textarea placeholder="Критерии AI-оценки" value={aiCriteria} onChange={e => setAiCriteria(e.target.value)} rows={2} />
            ...
            {isStudent && (
              <Dialog open={submitDialogOpen} onOpenChange={setSubmitDialogOpen}>
                ...
                <DialogContent>
                  ...
                  <Textarea placeholder="Ваш ответ" value={answerText} onChange={e => setAnswerText(e.target.value)} rows={5} />
            ...
            {submissions.map(s => {
              ...
              return (
                <div key={s.id} className="border border-border rounded-lg p-4">
                  ...
                  {s.answer_text && <p className="text-sm text-muted-foreground mb-2">{s.answer_text}</p>}
                  {s.ai_score != null && (
                    <div className="text-xs text-muted-foreground mb-1">AI оценка: <span className="font-medium text-foreground">{s.ai_score}</span> — {s.ai_comment}</div>
                  )}
